...
import { Typography, Skeleton } from "@douyinfe/semi-ui";
...
function LeaderBoard() {
	...
	const topMap = useQuery(
		["topDetailMap", topList],
		() => getTopDetail(topList),
		{
			// 当有榜单信息时再调用
			...
			>
				<div className="flex mt-5 px-32 justify-between">
					{topList.map((item) => {
						return (
							<div
								className="flex flex-col w-24/100 h-112 py-4 shadow-2xl cursor-pointer rounded-md"
								key={item.id}
								style={{
							...
							>
								<Title
									heading={4}
									className="text-white text-center font-mono italic"
								>
									{item.name}
								</Title>
